...
(def deck-of-54-cards (flatten [(range 1 11 1), "Jack", "Queen", "King"]))
...
(defn pick-from-54-deck
  "It pick a card from a 54 cards deck with two Jokers"
  []
  (if (< (rand-int 55) 2)
    (pick-joker)
...
(defn pick-card
  ...
  [type]
  (condp <= type
    54 (pick-from-54-deck)
    32 (pick-from-32-deck)
    (throw (Exception. "Invalid argument. Usage: pick-card [32 64]"))))
